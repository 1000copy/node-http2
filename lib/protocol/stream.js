...
// 流程转来转去的硬是也有点花哨。要搞懂本代码文件(不被end,duplex,_finish,_read,_write,_send,_receive 搞昏头），
// 估计必须搞懂stream的全双工。试读html：	http://codewinds.com/blog/2013-08-31-nodejs-duplex-streams.html
...
// 这里的stream，不是stream.js 自身，而是node内置类;Duplex 是一个  abstract class，
// 而不是一个interface，内有end方法，内部调用了_finish 。肯定的：）
/*
...
function Stream(log, connection) {
  //  this.id FROM Connection._allocateId
  Duplex.call(this);
...
Stream.prototype._writeUpstream = function _writeUpstream(frame) {
  ...
  function readPushPromise(buffer, frame) {
    var dataOffset = 0;
    var paddingLength = 0;
    if (false) {
  ...
  // function sanrex (frame){
  //   return
  //   console.log('by sanrex\n');
  //   var f={};
  //   f.type = frame.type;
  //   f.stream = frame.stream;
  //   if(frame.data) 
  //     f.data = frame.data.toString('utf8');
  //   // f.flags = frame.flags;
  //   if (frame.promised_stream)
  //     f.promised_stream = frame.promised_stream.id;
  //   //f.data1 = frame.data;
