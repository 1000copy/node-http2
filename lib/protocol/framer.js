...
当pipe两个流的后，上游来数据，就会在本节点调用 _transform，把流做转换，在push到自己的流内。
关联着这段代码来看才行。
Endpoint.prototype._initializeDataFlow 
...
// 注意，这里的_next ,虽然多了一个下划线，但不是override方法，只是一个普通方法：为了下一个frame设置状态，准备buffer。
...
Deserializer.prototype._transform = function _transform(chunk, encoding, done) {
  ...
  while(cursor < chunk.length) {
    ...
    if ((this._cursor === this._buffer.length) && !this._waitingForHeader) {
      ...
      this._next(COMMON_HEADER_SIZE);// 准备收frame HEADER 
    }
  }
...
//  三个字段的可打印名字
/*
'priorityDependency',  == Stream Dependency?
'priorityWeight',      == Weight
'exclusiveDependency'  == E 
*/
...
Serializer.PRIORITY = function writePriority(frame, buffers) {
  ...
  buffer.writeUInt32BE(frame.priorityDependency, 0);
  if (frame.exclusiveDependency) {
    buffer[0] |= 0x80; // 0x80 = 1000 0000 等于把buffer的最高位设置为1 
  }
  assert((0 <= frame.priorityWeight) && (frame.priorityWeight <= 0xff), frame.priorityWeight);
  //function prototype : writeUInt8(value ,offset)
  buffer.writeUInt8(frame.priorityWeight, 4);// 4 is offset 
