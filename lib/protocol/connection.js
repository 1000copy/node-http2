...
function Connection(log, firstStreamId, settings) {
  ...
  // this._streamSlotsFree: 流可以通过 SETTINGS_MAX_CONCURRENT_STREAMS 设置最大并行流，_streamSlotsFree用来记录还有多少新流可以创建。
...
既然 this.emit(frame.type, frame); 可以这样调用，显然->Flow->Deplex 是一个EventEmitter的子类。
...
// inbound 入境，outbound出境。怎么我脑袋里面总是什么“袋内、带外”之类的，被国内教材洗过脑。
...
// 在此准备要发送到对端（Peer）的数据Frame . 
// Callstack : Connection._send <-- (Parent Class)Flow._receive <- Flow._read <- Flow.read
...
Connection.prototype._initializeSettingsManagement = function _initializeSettingsManagement(settings) {
  ...
  this._log.debug({ settings: settings },
                  'Sending the first SETTINGS frame');
...
Connection.prototype._onFirstFrameReceived = function _onFirstFrameReceived(frame) {
  if ((frame.stream === 0) && (frame.type === 'SETTINGS')) {
    this._log.debug('Receiving the first SETTINGS frame ');
  } else {
