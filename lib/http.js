  ...
  // * HTTP/2 over TLS negotiated using NPN or ALPN, or fallback to HTTPS1
    ...
    */
    ...
    function negotiated() {      
      ...
      if (started) {
        // ** In the meantime/同时, an other connection was made to the same host...
        if (endpoint) {
        ...
        if (endpoint) {
          ...
          self.emit(key, endpoint);  // emit by here .this.once(key)内function执行和negotiated() 那个来的快。！！！！终于懂了。
        } else {
          self.emit(key, undefined);// 干掉once做的event关联
        }
      }
    }

    this.once(key, function(endpoint) { // event "key" emit by ---> negotiated() start = false ,emit (key)
      started = true;
      if (endpoint) {
...
function IncomingPromise(responseStream, promiseHeaders) {
  ...
  // console.log(responseStream.constructor.name=="Stream")
  ...
  IncomingRequest.call(this, responseStream);
